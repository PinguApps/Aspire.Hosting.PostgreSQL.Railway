...
Add-Pin -Path "Directory.Packages.props" -Name "Aspire.Hosting.PostgreSQL" -Version $packageVersions["Aspire.Hosting.PostgreSQL"]

$typeScriptConfigPaths = @(
  "samples/TypeScriptAppHost/aspire.config.json",
  "tests/Aspire.Hosting.PostgreSQL.Railway/Fixtures/TypeScriptAppHost/aspire.config.json"
)
...
foreach ($path in $typeScriptConfigPaths) {
  ...
  Add-Pin -Path $path -Name "packages.Aspire.Hosting.PostgreSQL" -Version $config.packages."Aspire.Hosting.PostgreSQL"
}
...
Add-RegexPin -Path "README.md" -Name "Aspire.Hosting.PostgreSQL sample" -Pattern '"Aspire\.Hosting\.PostgreSQL"\s*:\s*"(?<version>[^"]+)"'
Add-RegexPin -Path "docs/install.md" -Name "Aspire.Hosting.PostgreSQL install samples" -Pattern '"Aspire\.Hosting\.PostgreSQL"\s*:\s*"(?<version>[^"]+)"'
