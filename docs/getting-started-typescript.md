...
```json
{
  ...
  "Parameters": {
    "railway-database-name": "typescript-demo-cache",
    "railway-account-email": "demo@example.com",
    "railway-api-key": "demo-management-api-key"
  }
}
```

When validating this repository checkout instead of the published package, point `PinguApps.Aspire.Hosting.PostgreSQL.Railway` at the local project path instead of a version string.
...
Import the generated Railway value catalogs and extension methods from `./.aspire/modules/aspire.mjs`:
...
The maintained TypeScript demo is [`samples/TypeScriptAppHost/apphost.mts`](../samples/TypeScriptAppHost/apphost.mts). The tested fixture is [`tests/Aspire.Hosting.PostgreSQL.Railway/Fixtures/TypeScriptAppHost/apphost.mts`](../tests/Aspire.Hosting.PostgreSQL.Railway/Fixtures/TypeScriptAppHost/apphost.mts).
...
The local path continues to use standard Aspire Redis behaviour. Railway credentials are deploy-time inputs, but local demo commands are easier if `aspire.config.json` contains placeholder `Parameters` values like the example above.
...
- TypeScript has one `publishToRailway(databaseName, accountEmail, apiKey, options?)` method.
- The first three arguments are Aspire parameters.
- Optional settings are passed as a DTO object.
- Value catalogs such as `railwayPostgresOwnershipMode.createOrAdopt` are used instead of raw strings.
- Supplementary outputs are async getter methods from `await cache.getRailwayPostgresOutputs()`.
...
When a consumer needs individual Railway PostgreSQL output references:
...
Only the Redis password output is secret. The Railway Management API key is not an output.
