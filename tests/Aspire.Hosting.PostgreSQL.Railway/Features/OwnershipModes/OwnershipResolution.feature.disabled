Feature: Railway PostgreSQL ownership resolution

  Scenario: Create-only selects create when the named database is missing
    Given the Railway ownership resolver finds no database named "orders-cache"
  ...
  Scenario: Create-only fails when the named database already exists
    Given the Railway ownership resolver finds database "orders-cache" in region "eu-west-1" with TLS enabled
    When ownership is resolved for database "orders-cache" with mode "CreateOnly"
  ...
  Scenario: Create-only adopts the managed remote identity on repeated deploy
    Given the Railway ownership resolver finds database "orders-cache" in region "eu-west-1" with TLS enabled
    And the existing Railway database is the cached managed remote identity
    When ownership is resolved for database "orders-cache" with mode "CreateOnly"
  ...
  Scenario: Existing database with incompatible explicit settings fails clearly
    Given the Railway ownership resolver finds database "orders-cache" in region "us-east-1" with TLS enabled
  ...
  Scenario: Existing database with disabled TLS fails even when TLS is unset
    Given the Railway ownership resolver finds database "orders-cache" in region "eu-west-1" with TLS disabled
