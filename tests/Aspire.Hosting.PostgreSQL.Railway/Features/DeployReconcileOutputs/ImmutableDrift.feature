Feature: Immutable Railway PostgreSQL drift detection

  Scenario: Database name identity drift fails before mutation
    Given an existing Railway PostgreSQL database detail named "renamed-cache" in region "eu-west-1" with TLS enabled
  ...
  Scenario: Platform drift fails when provider state exposes the platform through primary region
    Given an existing Railway PostgreSQL database detail named "orders-cache" in region "us-central1" with TLS enabled
  ...
  Scenario: Primary region drift fails for explicit requested primary region
    Given an existing Railway PostgreSQL database detail named "orders-cache" in region "us-east-1" with TLS enabled
  ...
  Scenario: Disabled remote TLS fails even when TLS was not explicit
    Given an existing Railway PostgreSQL database detail named "orders-cache" in region "eu-west-1" with TLS disabled
    When immutable drift is checked for requested database "orders-cache" with default options
  ...
  Scenario: Mutable setting differences do not fail immutable drift detection
    Given an existing Railway PostgreSQL database detail named "orders-cache" in region "eu-west-1" with TLS enabled
