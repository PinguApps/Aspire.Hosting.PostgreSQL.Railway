...
public sealed class TypeScriptAppHostFixtureStepDefinitions
{
    ...
    [Then("the fixture imports the generated Aspire and Railway PostgreSQL modules")]
    public void ThenTheFixtureImportsTheGeneratedAspireAndRailwayPostgresModules()
    {
        string source = GetAppHostSource();

        Assert.Contains("from \"./.aspire/modules/aspire.mjs\"", source, StringComparison.Ordinal);
        Assert.Contains("railwayPostgresCloudPlatform", source, StringComparison.Ordinal);
        Assert.Contains("railwayPostgresOwnershipMode", source, StringComparison.Ordinal);
        Assert.Contains("railwayPostgresPlan", source, StringComparison.Ordinal);
        Assert.Contains("railwayPostgresRegion", source, StringComparison.Ordinal);
    }
    ...
    public void ThenTheFixtureCreatesABuilderAndRedisResource()
    {
        ...
        Assert.Contains("await builder.addParameter(\"railway-database-name\")", source, StringComparison.Ordinal);
        Assert.Contains("await builder.addParameter(\"railway-account-email\")", source, StringComparison.Ordinal);
        Assert.Contains("await builder.addParameter(\"railway-api-key\", { secret: true })", source, StringComparison.Ordinal);
    }

    [Then("the fixture calls the generated Railway PostgreSQL publish API with DTO options")]
    public void ThenTheFixtureCallsTheGeneratedRailwayPostgresPublishApiWithDtoOptions()
    {
        string source = GetAppHostSource();

        Assert.Contains("cache.publishToRailway(databaseName, accountEmail, apiKey", source, StringComparison.Ordinal);
        Assert.Contains("ownershipMode: railwayPostgresOwnershipMode.createOrAdopt", source, StringComparison.Ordinal);
        Assert.Contains("platform: railwayPostgresCloudPlatform.aws", source, StringComparison.Ordinal);
        Assert.Contains("primaryRegion: railwayPostgresRegion.awsEuWest1", source, StringComparison.Ordinal);
        Assert.Contains("readRegions: [railwayPostgresRegion.awsEuWest2]", source, StringComparison.Ordinal);
        Assert.Contains("plan: railwayPostgresPlan.payAsYouGo", source, StringComparison.Ordinal);
    ...
    [Then("the fixture consumes the generated Railway PostgreSQL outputs")]
    public void ThenTheFixtureConsumesTheGeneratedRailwayPostgresOutputs()
    {
        string source = GetAppHostSource();

        Assert.Contains("const outputs = await cache.getRailwayPostgresOutputs();", source, StringComparison.Ordinal);
    ...
    [Then("the generated TypeScript SDK exposes the Railway PostgreSQL surface")]
    public void ThenTheGeneratedTypeScriptSdkExposesTheRailwayPostgresSurface()
    {
        ...
        Assert.Contains("publishToRailway", generatedSource, StringComparison.Ordinal);
        Assert.Contains("getRailwayPostgresOutputs", generatedSource, StringComparison.Ordinal);
        Assert.Contains("railwayPostgresCloudPlatform", generatedSource, StringComparison.Ordinal);
        Assert.Contains("railwayPostgresOwnershipMode", generatedSource, StringComparison.Ordinal);
        Assert.Contains("railwayPostgresPlan", generatedSource, StringComparison.Ordinal);
        Assert.Contains("railwayPostgresRegion", generatedSource, StringComparison.Ordinal);
    }
    ...
    private static string FindFixtureDirectory()
    {
        ...
        while (directory is not null)
        {
            string candidate = Path.Combine(
                directory.FullName,
                "tests",
                "Aspire.Hosting.PostgreSQL.Railway",
                "Fixtures",
                "TypeScriptAppHost");
        ...
        throw new DirectoryNotFoundException("Could not find tests/Aspire.Hosting.PostgreSQL.Railway/Fixtures/TypeScriptAppHost.");
    }
