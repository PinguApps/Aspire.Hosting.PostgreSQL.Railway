...
- `RailwayPostgresScenarioContext` is the per-scenario object model for Aspire builders, captured options, fake provider state, and live-provider cleanup.
...
- `FakeRailwayProvider` is the default Railway simulation path. It is deterministic, in memory, and records provider interactions for assertions.
- `LiveRailwayTestSession` is the opt-in live-provider pattern. Live scenarios must use the `@live-railway` tag, read only `RAILWAY_EMAIL` and `RAILWAY_API_KEY`, and register every delete or restore action through the shared cleanup stack. Cleanup runs best-effort through the full stack and reports failures after all actions have been attempted.
...
- Normal CI excludes `@live-railway` scenarios and runs the NuGet-backed TypeScript AppHost package gate.
...
- `6.1`: covered in `Features/LocalBehavior/RedisReference.feature` and `Features/ApiShape/PublishToRailway.feature` for plain `AddRedis` no-op behavior, local pre-deploy Railway no-op behavior, standard Redis references, overload consistency, same-builder fluent chaining, and management-secret exclusion from app-facing Redis outputs/references.
...
- `6.3`: covered in `Features/DeployReconcileOutputs` for per-setting mutable reconciliation, TLS non-mutation, adopted missing-credential, output missing-password failures, no automatic `reset-password` on missing credentials, secret redaction, and opt-in `@live-railway` disposable deploy/repeat-deploy/output scenarios. Live 6.3 scenarios generate isolated database names and register provider deletion by configured name through the shared cleanup stack before deployment runs.
- `7.2`: covered in `Features/DocsSamples/AppHostSnippets.feature` by linking `samples/AppHostSnippets/RailwayPostgresAppHostSnippets.cs` into the test project and invoking each snippet against a fresh Aspire builder.
- `7.1`: align README snippets to `samples/AppHostSnippets/RailwayPostgresAppHostSnippets.cs` where practical and add DocsSamples coverage only for README-specific drift checks that are not already covered by the compiled sample source.
- `10.1`: covered in `Features/ApiShape/TypeScriptAppHostFixture.feature` by validating the checked-in TypeScript AppHost fixture consumes the intended generated module surface, calls `publishToRailway`, consumes supplementary outputs, preserves normal Redis references, restores the generated SDK, type-checks the generated surface, starts locally with Redis readiness handling, lists publish steps, and keeps generated modules out of source control. `Features/DeployReconcileOutputs/TypeScriptDeployment.feature` adds fake-provider repeated-deploy and gated `@live-railway` validation through the TypeScript bridge and existing deployment pipeline.
