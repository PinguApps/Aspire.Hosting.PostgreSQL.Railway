...
internal sealed class RailwayPostgresReconciler
{
    ...
    public async Task<RailwayPostgresDatabaseDetails> ReconcileAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        RailwayPostgresDatabaseDetails current = database;
    ...
    private async Task<RailwayPostgresDatabaseDetails> ReconcileReadRegionsAsync(
        RailwayPostgresDatabaseDetails current,
        RailwayPostgresProviderDeploymentOptions options,
        CancellationToken cancellationToken)
    {
        if (!options.ExplicitSettings.Contains(nameof(RailwayPostgresDeploymentOptions.ReadRegions))
        ...
        try
        {
            await _client.UpdateReadRegionsAsync(
                current.DatabaseId,
                new RailwayPostgresUpdateRegionsRequest { ReadRegions = desiredReadRegions },
                cancellationToken).ConfigureAwait(false);

            return await _client
                .WaitUntilReadyAsync(current.DatabaseId, RailwayPostgresReadinessPollingOptions.Default, cancellationToken)
    ...
    private async Task<RailwayPostgresDatabaseDetails> ReconcilePlanAsync(
        RailwayPostgresDatabaseDetails current,
        RailwayPostgresProviderDeploymentOptions options,
        CancellationToken cancellationToken)
    {
        if (!options.ExplicitSettings.Contains(nameof(RailwayPostgresDeploymentOptions.Plan))
        ...
        try
        {
            await _client.ChangePlanAsync(
                current.DatabaseId,
                new RailwayPostgresChangePlanRequest { PlanName = desiredPlan },
                cancellationToken).ConfigureAwait(false);
    ...
    private async Task<RailwayPostgresDatabaseDetails> ReconcileBudgetAsync(
        RailwayPostgresDatabaseDetails current,
        RailwayPostgresProviderDeploymentOptions options,
        CancellationToken cancellationToken)
    {
        if (!options.ExplicitSettings.Contains(nameof(RailwayPostgresDeploymentOptions.Budget))
        ...
        try
        {
            await _client.UpdateBudgetAsync(
                current.DatabaseId,
                new RailwayPostgresUpdateBudgetRequest { Budget = desiredBudget },
                cancellationToken).ConfigureAwait(false);
    ...
    private async Task<RailwayPostgresDatabaseDetails> ReconcileEvictionAsync(
        RailwayPostgresDatabaseDetails current,
        RailwayPostgresProviderDeploymentOptions options,
        CancellationToken cancellationToken)
    {
        if (!options.ExplicitSettings.Contains(nameof(RailwayPostgresDeploymentOptions.Eviction))
    ...
    private static string GetRequiredStringLiteral(RailwayPostgresProviderValue value)
    {
        return value.LiteralValue as string
            ?? throw new InvalidOperationException("Resolved Railway PostgreSQL deployment options must contain string provider values before reconciliation.");
    }

    private static int GetRequiredIntLiteral(RailwayPostgresProviderValue value)
    {
        return value.LiteralValue is int literalValue
            ? literalValue
            : throw new InvalidOperationException("Resolved Railway PostgreSQL deployment options must contain integer provider values before reconciliation.");
    }

    private static bool GetRequiredBoolLiteral(RailwayPostgresProviderValue value)
    {
        return value.LiteralValue is bool literalValue
            ? literalValue
            : throw new InvalidOperationException("Resolved Railway PostgreSQL deployment options must contain boolean provider values before reconciliation.");
    }
    ...
    private static string FormatPlan(RailwayPostgresDatabaseDetails current)
    {
    ...
    private static bool PlanMatches(string desiredPlan, RailwayPostgresDatabaseDetails current)
    {
    ...
    private static RailwayPostgresReconciliationException CreateMutationException(
        RailwayPostgresDatabaseDetails current,
        string settingName,
        string desiredValue,
        RailwayPostgresProviderException exception)
    {
        return new RailwayPostgresReconciliationException(
            settingName,
            exception.FailureKind,
            $"Failed to reconcile Railway PostgreSQL database '{current.DatabaseName}' setting '{settingName}' to '{desiredValue}'. {exception.Message}",
            exception);
    }

    private static RailwayPostgresReconciliationException CreateVerificationException(
        RailwayPostgresDatabaseDetails current,
        string settingName,
        string desiredValue,
        string actualValue)
    {
        return new RailwayPostgresReconciliationException(
            ...
            $"Railway PostgreSQL database '{current.DatabaseName}' did not converge after reconciling setting '{settingName}'. Requested '{desiredValue}', found '{actualValue}'.");
    }
}
